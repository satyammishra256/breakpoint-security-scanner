(() => {
  ...
  function mountAccount(){if(location.pathname.includes('/auth/'))return;const accBtn=document.getElementById('bp-header-account'),setBtn=document.getElementById('bp-header-settings');if(accBtn)accBtn.onclick=()=>token()?showAccount():go('auth');if(setBtn)setBtn.onclick=showSettings}
