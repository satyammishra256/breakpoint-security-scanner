(() => {
  ...
  async function runValidation() {
    if (!state.projectId) { toast('Create a project first.', false); return; }
    try {
      const r = await json(`/projects/${state.projectId}/validation`, { method: 'POST' });
      toast(`Validation complete: ${r.risk_before.toUpperCase()} → ${r.risk_after.toUpperCase()}`);
  ...
  async function loadVulnerabilities() {
    ...
    try {
      ...
      list.querySelectorAll('[data-v]').forEach(b => b.onclick = () => { localStorage.setItem('BREAKPOINT_VULN_ID', b.dataset.v); go('vulnerability'); });
